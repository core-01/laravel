...
class AdminViewController extends Controller
{
    ...
    public function emiTransactionHistory(Request $request){
        ...
        if($request->fillter_value){
            ...
            $exists =EmiDetail::where('username',$request->fillter_value)->get();
            if(count($exists) <= 0){
        ...
        $payment_history = EmiDetail::all();
        if($request->fillter_value){
            if($request->fillter_by == "Mobile"){

                $payment_history = EmiDetail::where('username',$request->fillter_value)->get();
                $select_type ="Mobile";
                $fillter_value = $request->fillter_value;
            }else if($request->fillter_by == "Transaction Id"){
                $payment_history = EmiDetail::where('transaction_id',$request->fillter_value)->get();
                $select_type ="transaction_id";
                $fillter_value = $request->fillter_value;


            }else if($request->fillter_by == "Date"){
                $payment_history = EmiDetail::where('transaction_date',date('Y-m-d',strtotime($request->fillter_value)))->get();
                $select_type ="date";
                $fillter_value = $request->fillter_value;
        ...
        return view('admin.transaction-history',compact('payment_history','select_type','fillter_value'));
    }
    ...
    public function loanrequest(){
        $loanReq = LoanRequest::orderBy('id', 'DESC')->get();
    ...
    public function loanAproved(){
        $loanReq = LoanRequest::where('user_status','Accepted')->orderBy('id', 'DESC')->get();
        $bankDetail = BankDetail::all();
        // $bankDetail->loan_request_number
        // dd()
